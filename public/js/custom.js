// Виконати, коли HTML-документ буде повністю завантажено
document.addEventListener('DOMContentLoaded', function () {

    // Отримати календар SELECTOR з атрибуту id
    var calendarEl = document.getElementById('calendar');

    // Отримати SELECTOR з модального вікна регістру
    const cadastrarModal = new bootstrap.Modal(document.getElementById("cadastrarModal"));

    // Отримати SELECTOR з представлення модального вікна
    const visualizarModal = new bootstrap.Modal(document.getElementById("visualizarModal"));

    // Отримати селектор «msgViewEvent»
    const msgViewEvento = document.getElementById('msgViewEvento');

    // Створіть екземпляр FullCalendar.Calendar і присвойте змінній calendar
    var calendar = new FullCalendar.Calendar(calendarEl, {

        // увімкнути bootstrap 5
        themeSystem: 'bootstrap5',
        // Створіть заголовок календаря
        headerToolbar: {
        ...
        // Дозволяє клацнути і перетягнути мишею один або кілька днів у календарі
        selectable: true,

        // Візуально позначте область, яку потрібно вибрати, перш ніж користувач відпустить кнопку миші, щоб підтвердити вибір
        selectMirror: true,

        // Дозволяє перетягувати та змінювати розміри подій безпосередньо в календарі.
        editable: true,

        // Максимальна кількість подій у певний день, якщо значення true, то кількість подій буде обмежено висотою комірки дня
        dayMaxEvents: true,
    ...
    if (formCadEvento) {
        ...
        formCadEvento.addEventListener("submit", async (e) => {
            ...
            const dados = await fetch("/event/store", {
                method: "POST",
                headers: {
    ...
    if (formEditEvento) {
        ...
        formEditEvento.addEventListener("submit", async (e) => {
            ...
            const dados = await fetch(`/api/events/${id}`, {
    ...
    if (btnApagarEvento) {
        ...
        btnApagarEvento.addEventListener("click", async () => {
            ...
            const confirmacao = window.confirm("Ви впевнені, що хочете видалити цю подію?");
            ...
            if (confirmacao) {
                ...
                const dados = await fetch(`/api/events/${idEvento}`, {
                    method: "DELETE",
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                    }
